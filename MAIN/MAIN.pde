...
public class Waves extends Visual {
    ...
    public Waves(AudioSource[] str, Boolean c){
      super(str);
...
public class Waves2 extends Visual {
    ...
    public class Mountain{
      ...
      public Mountain(int i, int cx, int cy, int w, int mh, AudioSource s){
        idx = i;
        center_x = cx;
        center_y = cy;
        max_height = mh;
      ...
      public void draw(){
        float current_height = max_height * src.volume * 2;
        pg.pushMatrix();
        pg.circle(center_x + src.volume*100,center_y  + src.volume*100, base_width);
        pg.popMatrix();
        //pg.rotateZ((clock + idx)/100000.);
        for (int r = 1; r < rings; r++){
          float trans = current_height/(rings - r);
          pg.pushMatrix();
          pg.translate(0,0, trans);
          pg.stroke(src.volume*2550, src.spectrum[0]*2550/(r + 1), src.spectrum[2]*2550/(r + 1));
          pg.fill(src.volume*2550, src.spectrum[0]*2550/(r + 1), src.spectrum[2]*2550/(r + 1));
          pg.circle(center_x  + src.volume*100,center_y  + src.volume*100, base_width/(r+1));
          pg.translate(0,0, -trans);
          pg.popMatrix();
        }
      }
    ...
    public Waves2(AudioSource[] str){
      super(str);
      pg = createGraphics(width, height, P3D);
      mtns = new Mountain[nMountain];
      for (int i = 0; i < nMountain; i++){
        mtns[i] = new Mountain(i, rand.nextInt(width), rand.nextInt(height), rand.nextInt(100),  rand.nextInt(100), sources[i % nInputs]);
      }
    }
    ...
    public void updateGraphic(){
      ...
      for(int x = 0; x < size; x++){
        for(int y = 0; y < size; y++){
          float posx = x*(width/size);
          float posy = y*(height/size);
          og.fill(sin((x + clock)/1000.)*sources[(x + clock/1000) % nInputs].spectrum[(y) % bands]*25500, sin((y + clock)/1000.)*sources[(x + clock/1000) % nInputs].spectrum[(y+1) % bands]*25500, sources[(x + clock/1000) % nInputs].spectrum[(y+2) % bands]*2550);
          og.rect(posx % width,posy,posx+(x+1)*(width/size),posy+(y+1)*(height/size));
        }
      }
      og.endDraw();
      og.loadPixels();
      ...
      for(int x = 0; x < width; x++){
        for(int y = 0; y < height; y++){
         int offset = x + y*width;
         int oldset = parseInt(min(width - 1, x+sources[y % nInputs].volume*100)) + parseInt(min((y+sources[x % nInputs].volume*50), height - 1))*width;
         pg.pixels[offset] = og.pixels[oldset];
        }
      }
      pg.updatePixels();
...
void setup(){
  ...
  visuals[3] = new Waves(sources, true);
  visuals[4] = new GrowingShapes(sources);
  visuals[5] = new Grid(sources);
...
void keyReleased() {
  if (key == 'd') {
    ...
    if(currentVisual == 3){
      visuals[currentVisual].setLast(visuals[currentVisual - 1].pg);
    }
  } else if (key == 'a'){
    currentVisual--;
    clock = 0;
